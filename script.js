...
const ACH_ICONS = {
  ...
  shuffle:`<path d="M16 3h5v5"/><path d="M4 20 21 3"/><path d="M21 16v5h-5"/><path d="M15 15 21 21"/><path d="M4 4 9 9"/>`,
...
function renderMiniQuiz(containerId, lesson){
  ...
  const quizQs = shuffle(lesson.quiz).map(shuffleQuestion);
  let idx = 0, correctCount = 0;
  function renderQ(){
    if(idx >= quizQs.length){
      container.innerHTML = `<div class="feedback pass">Nice — you got ${correctCount}/${quizQs.length} right.</div>`;
      const list = STATE.quizAttempts[lesson.id] || [];
      list.push({score:correctCount, total:quizQs.length, date:new Date().toLocaleDateString()});
    ...
    container.querySelectorAll(".mc-opt").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        ...
        document.getElementById("miniQFeedback").innerHTML = `<div class="feedback ${i===q.correct?'pass':'fail'}">${q.explain}</div>
          <button class="btn small secondary" style="margin-top:10px;" onclick="(${renderQ.toString().replace(/"/g,'&quot;')})()" id="nextMiniQ">${idx+1<quizQs.length?'Next question':'See result'}</button>`;
        idx++;
...
function renderQuizHome(){
  const html = [`<div class="grid grid-3" style="margin-bottom:20px;">`,
    `<button class="card" style="text-align:left;border:1px solid var(--border);" onclick="startQuiz('mixed')">
      <div style="font-weight:800;font-size:14.5px;display:flex;align-items:center;gap:6px;">${icon('shuffle',15)}Mixed Review</div>
...
// Returns a copy of a quiz question with its answer options reshuffled,
// so the correct answer doesn't always land in the same position.
...
function startQuiz(scope){
  ...
  const qs = shuffle(pool).slice(0,5).map(shuffleQuestion);
  let idx=0, correct=0;
...
/* ============================================================
   AI TUTOR   (calls our own /api/tutor backend, which holds the
   real Anthropic API key server-side — see server.js)
   ============================================================ */
let tutorHistory = [];
...
async function tutorSend(){
  ...
  try{
    // Same-origin call — no API key here. The server attaches the key
    // and the system prompt, then forwards the request to Anthropic.
    const response = await fetch("/api/tutor", {
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ messages: tutorHistory })
    });
